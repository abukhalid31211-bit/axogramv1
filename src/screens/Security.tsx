...
function Blacklist() {
  ...
  return (
    <div className="animate-fade">
      ...
      {blacklist.length === 0 ? <EmptyState icon={<Ban className="h-8 w-8" />} title="القائمة فارغة" />
      ...
      <ConfirmDialog open={confirmClear} danger title="مسح القائمة" message="سيتم حذف جميع المستخدمين المحظورين."
        onConfirm={() => { setConfirmClear(false); show("تم المسح","danger"); }} onCancel={() => setConfirmClear(false)} />
      {node}
    </div>
  );
}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="card p-5">
          ...
          <div className="space-y-2">
            ...
            <OptionButton label="متوازن (موصى)" badge={<span className="chip bg-brand-50 text-brand-700 ring-1 ring-brand-200">موصى</span>} selected={level === "balanced"} onClick={() => setLevel("balanced")} />
