...
export function MassDmModule() {
  ...
  const items = [
    ...
    { id: "blacklist", label: "القائمة السوداء (DM)", desc: "مستخدمون محظورون", icon: Ban },
    ...
    <div className="animate-fade">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((it) => {
          const Icon = it.icon;
          return (
            <button key={it.id} onClick={() => push(["massdm", it.id])} className="card flex items-center gap-3 p-4 text-right hover:border-brand-500/40 hover:shadow-glow">
...
function NewDm() {
  ...
  const [protection, setProtection] = useState({ save: true, log: true, notify: true, stopFail: true, deleteAfter: false });
      ...
      <div className="mx-auto max-w-2xl">
          ...
          <div className="card p-6 space-y-4">
            ...
            {source === "csv" && (
              <div className="rounded-xl border border-ink-700/60 p-3">
                <div className="mb-2 text-xs text-slate-400">إجمالي: 1,000 | بـ username: 800 | بهاتف: 200</div>
        ...
        {step === 2 && (
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <OptionButton label="30/يوم (متوازن)" badge={<span className="chip bg-brand-500/15 text-brand-300">موصى</span>} selected={dailyLimit === "30"} onClick={() => setDailyLimit("30")} />
        ...
        {step === 4 && (
          <div className="card p-6 space-y-4">
            ...
            <SectionTitle>عند User not found</SectionTitle>
            <div className="grid grid-cols-2 gap-2">
              <OptionButton label="تخطي تلقائياً" badge={<span className="chip bg-brand-500/15 text-brand-300">موصى</span>} selected={true} onClick={() => {}} />
              <OptionButton label="تسجيل فقط" onClick={() => {}} />
            </div>
        ...
        {step === 5 && (
          <div className="card p-6">
            ...
            <div className="mb-4 grid gap-2 text-sm sm:grid-cols-2">
              <SummaryRow label="مستلمون" value="1,000" />
...
function ListDm() {
  ...
  return (
    <div className="animate-fade">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={<MessageSquare className="h-8 w-8" />} title="لا توجد حملات" />
      ) : (
            ...
            <div key={c.id} className="card p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-slate-400">{c.date} • {c.recipients.toLocaleString()} مستلم</div>
                </div>
  ...
  return (
    <div className="animate-fade">
      ...
      <div className="mx-auto max-w-lg card p-6 space-y-3">
        <Field label="التأخير الافتراضي (ث)" value={delay} onChange={setDelay} />
        ...
        <Button variant="primary" className="w-full" onClick={() => { show("تم الحفظ"); push(["massdm"]); }}>حفظ</Button>
      </div>
      {node}
    </div>
  );
