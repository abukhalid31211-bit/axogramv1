...
import { PageHeader, Button, Field, Checkbox, OptionButton, Progress, Table, SectionTitle, Alert, useToast, EmptyState, InlineEdit } from "../ui";
...
function AddFromCsv() {
  ...
  const [accs, setAccs]   = useState<"single"|"rotate"|"custom">("rotate");
  ...
  const [protection, setProtection] = useState({ skipExisting: true, skipBlacklist: true, saveProgress: true, floodWait: true });
      ...
      <div className="mx-auto max-w-2xl">
        {step === 0 && (
          <div className="card p-6 space-y-4">
            ...
            <div className="space-y-2">{exportedFiles.map((f) => <OptionButton key={f.id} label={f.name} desc={`${f.members.toLocaleString()} عضو`} selected={file === f.id} onClick={() => setFile(f.id)} />)}</div>
        ...
        {step === 1 && (
          <div className="card p-6 space-y-4">
            ...
            <SectionTitle>الحسابات</SectionTitle>
            <div className="grid grid-cols-3 gap-2">
              <OptionButton label="حساب واحد"        selected={accs === "single"} onClick={() => setAccs("single")} />
              <OptionButton label="تدوير بين النشطة"  selected={accs === "rotate"} onClick={() => setAccs("rotate")} />
              <OptionButton label="تحديد حسابات"     selected={accs === "custom"} onClick={() => setAccs("custom")} />
            </div>
            ...
            {delay === "custom" && <InlineEdit label="التأخير (ثانية)" value="45" onSave={setDelay} placeholder="45" />}
            ...
            {dailyLimit === "custom" && <InlineEdit label="الحد اليومي المخصص" value="35" onSave={setDailyLimit} placeholder="35" />}
        ...
        {step === 2 && (
          <div className="card p-6">
            ...
            <div className="mb-4 grid gap-2 text-sm sm:grid-cols-2">
              ...
              <SRow label="طريقة"    value="مباشرة" />
              <SRow label="حسابات"   value="تدوير" />
              <SRow label="تأخير"    value={`${delay} ث`} />
              <SRow label="حد يومي"  value={`${dailyLimit}/يوم`} />
            </div>
            <Alert tone="info" title="تقدير: 60 إضافة/يوم | 235 يوم للاكتمال" />
            {!running && progress === 0 && <Button variant="primary" className="mt-4 w-full" icon={<Play className="h-4 w-4" />} onClick={start}>بدء الإضافة الآن</Button>}
            {(running || progress > 0) && progress < 100 && (
              <div className="mt-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-center text-xs text-surface-500 sm:grid-cols-4">
                  <div>+966501234567</div><div>أُضيف: {Math.floor(progress * 1.4)}</div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <Button icon={<Plus className="h-4 w-4" />} onClick={() => show("+30%")}>+30%</Button>
            ...
            {progress === 100 && (
              <div className="mt-4 space-y-3">
                <Alert tone="success" title="اكتملت الإضافة">✅ 13,500 ناجح | ⚠️ 500 تخطي | ❌ 135 فاشل</Alert>
