...
import { PageHeader, Button, Field, OptionButton, Table, SectionTitle, useToast } from "../ui";
    ...
    <div className="animate-fade">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((it) => {
          const Icon = it.icon;
          return (
            <button key={it.id} onClick={() => push(["rotation", it.id])} className="card flex items-center gap-3 p-4 text-right hover:border-brand-500/40 hover:shadow-glow">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="space-y-2">
            ...
            <OptionButton label="ذكي (حالة+استخدام+عمر)" badge={<span className="chip bg-brand-500/15 text-brand-300">موصى</span>} selected={mode === "smart"} onClick={() => setMode("smart")} />
          </div>
        </div>
        <div className="card p-5">
...
function RotationTable() {
  ...
  return (
    <div className="animate-fade">
      ...
      <Table columns={["#", "حساب", "وضع", "التالي"]} rows={accounts.slice(0, 5).map((a, i) => [i + 1, a.phone, <span className="text-brand-300">نشط</span>, i < 4 ? accounts[i + 1].phone : accounts[0].phone])} />
  ...
  return (
    <div className="animate-fade">
      ...
      <div className="mx-auto max-w-lg card p-6 space-y-4">
        ...
        <div className="text-sm text-slate-300">1 → 2 → 3 → 4 → 5</div>
        <Field label="الترتيب الجديد (أرقام/فواصل)" value={order} onChange={setOrder} />
...
function DailyUsage() {
  ...
  return (
    <div className="animate-fade">
      ...
      <Table columns={["حساب", "تجميع", "إضافة", "رسائل", "حالة"]} rows={accounts.slice(0, 5).map((a) => [a.phone, "120", "18", "8", <span className="text-brand-300">نشط</span>])} />
      <div className="mt-3 flex items-center justify-between rounded-xl bg-ink-850/50 px-4 py-3 text-sm">
        <span className="text-slate-400">إجمالي اليوم</span>
        <span className="font-bold text-white">600 عملية | تصفير: 00:00</span>
      </div>
      <div className="mt-4"><Button onClick={() => push(["rotation"])}>رجوع</Button></div>
    </div>
  );
...
function ResetCounters() {
  ...
  return (
    <div className="animate-fade">
      ...
      <div className="mx-auto max-w-lg card p-6 space-y-4">
        <p className="text-sm text-slate-300">سيتم تصفير جميع عدادات الاستهلاك اليومي.</p>
        <div className="flex gap-2">
