      ...
      <div className="mx-auto max-w-lg">
        ...
        {step === 4 && (
          <div className="card p-6 text-center">
            <div className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-full bg-brand-50 text-brand-600 ring-2 ring-brand-200 animate-pulse-glow"><CheckCircle2 className="h-7 w-7" /></div>
            <h3 className="text-lg font-bold text-surface-800">تم تسجيل الدخول بنجاح</h3>
            <p className="mt-1 text-sm text-surface-500">الحساب {phone || "+9665XXXXXXXX"} جاهز للاستخدام.</p>
...
function ListAccounts() {
  ...
  return (
    <div className="animate-fade">
      ...
      <Table columns={["","رقم","اسم","حالة","بروكسي","آخر استخدام",""]}
        rows={filtered.map((a) => [
      ...
      <ConfirmDialog open={confirmDel} danger title="حذف الحسابات المحددة" message={`سيتم حذف ${selected.length} حساب نهائياً.`} confirmLabel="تأكيد الحذف"
        onConfirm={() => { setConfirmDel(false); setSelected([]); show("تم حذف الحسابات"); }} onCancel={() => setConfirmDel(false)} />
      {node}
    </div>
  );
}
...
export function AccountDetail({ id }: { id: string }) {
  ...
  return (
    <div className="animate-fade">
      <PageHeader title={`حساب: ${name}`} subtitle={acc0.phone} icon={<Users className="h-5 w-5" />} />
      <div className="grid gap-4 md:grid-cols-2">
        <div className="card p-5">
          <SectionTitle>بطاقة الحساب</SectionTitle>
          <div className="space-y-2">
            <InlineEdit label="الاسم"     value={name}     onSave={setName}     placeholder="الاسم الأول" />
            <InlineEdit label="@username" value={username} onSave={setUsername} placeholder="@username" />
            <InlineEdit label="البروكسي"  value={proxy}    onSave={setProxy}    placeholder="IP:PORT" />
            <Row label="الهاتف"       value={acc0.phone} />
            <Row label="العمر"         value={acc0.age} />
        ...
        <div className="card p-5">
          ...
          <div className="grid gap-2">
            ...
            <Button icon={<ShieldCheck className="h-4 w-4" />} onClick={() => show("الحساب نشط")}>التحقق من هذا الحساب</Button>
      ...
      <ConfirmDialog open={confirmDel} danger title="إزالة الحساب" message={`سيتم حذف ${acc0.phone} نهائياً.`}
        onConfirm={() => { setConfirmDel(false); show("تم حذف الحساب"); push(["accounts","list"]); }} onCancel={() => setConfirmDel(false)} />
      {node}
    </div>
  );
}
            ...
            <div className="space-y-4">
              <Alert tone="success" title="اكتمل التحقق">
                <div className="mt-2 flex gap-3">
                  ...
                  <span className="chip bg-warn-50 text-warn-700 ring-1 ring-warn-200">مقيد: 1</span>
                </div>
              </Alert>
  ...
  return (
    <div className="animate-fade">
      ...
      <ConfirmDialog open={confirm} danger title="تأكيد حذف الحساب" message={`سيتم حذف ${val} نهائياً.`}
        onConfirm={() => { setConfirm(false); show("تم حذف الحساب"); push(["accounts","list"]); }} onCancel={() => setConfirm(false)} />
      {node}
    </div>
  );
}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="card p-5">
          ...
          <div className="space-y-2">
            ...
            <OptionButton label="مكثفة — 7 أيام (موصى)" badge={<span className="chip bg-brand-50 text-brand-700 ring-1 ring-brand-200">موصى</span>} selected={intensity === "intensive"} onClick={() => setIntensity("intensive")} />
          </div>
